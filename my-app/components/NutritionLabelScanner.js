...
const REQUIRED_FIELDS = ["calories", "protein", "carbs", "fats", "sugar"];

function normalizeDetectedNutrition(result = {}) {
  const calories = Number(result.calories);
  const protein = Number(result.protein);
  const carbs = Number(result.carbohydrates ?? result.carbs);
  const fats = Number(result.fat ?? result.fats);
  const sugar = Number(result.sugar);
  ...
  return {
    ...
    fat: Number.isFinite(fats) ? fats : null,
    carbohydrates: Number.isFinite(carbs) ? carbs : null,
    sugar: Number.isFinite(sugar) ? sugar : null,
  };
}
...
export default function NutritionLabelScanner({ onDetected }) {
  ...
  async function handleFileChange(event) {
    ...
    if (!isLikelyImageFile(file)) {
      setToast(
        buildToast("Please choose a photo of a nutrition label.", "danger"),
      );
      return;
    }
    ...
    try {
      ...
      console.info(
        `${logId}: compressed scan file size ${(preparedFile.size / 1024).toFixed(1)} KB`,
      );
      ...
      const normalizedResult = normalizeDetectedNutrition(result);
      onDetected?.(normalizedResult);
      const missingFields = getMissingFields(normalizedResult);
      if (missingFields.length > 0) {
        setScanNote(
          `Please double-check scanned values. Missing: ${missingFields.join(", ")}.`,
        );
        setToast(
          buildToast(
            "Some values were not detected. Please review the form.",
            "danger",
          ),
        );
      } else {
        ...
        setToast(
          buildToast(
            "Nutrition values detected. Please review them before saving.",
            "success",
          ),
        );
      }
  ...
  return (
    <>
      ...
      <ToastContainer
        position="top-end"
        className="nutrition-scan-toast-container"
      >
        <Toast
          show={toast.show}
